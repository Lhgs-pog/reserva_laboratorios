...
class Agendamento extends BaseModel {
    ...
    /**
     * Lista reservas pendentes de um professor
     */
    public function listarPendentesProfessor($id_professor) {
        $sql = "SELECT a.id, l.nome as laboratorio, d.nome as disciplina, a.turno, a.periodo, a.data_reserva 
                FROM {$this->table} a
                INNER JOIN laboratorios l ON a.id_laboratorio = l.id
                INNER JOIN disciplinas d ON a.id_disciplina = d.id
                WHERE a.id_professor = :id_professor AND a.status = 'pendente'
                ORDER BY a.data_reserva ASC";
