...
CREATE TABLE IF NOT EXISTS `chamados_suporte` (
  ...
  `status`        ENUM('pendente','em_andamento','aguardando_verificacao','resolvido','nao_resolvido') NOT NULL DEFAULT 'pendente',
  `data_hora`     DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `observacao_interna` TEXT NULL,
  `resposta_professor` TEXT NULL,
  `id_atendente`  INT UNSIGNED NULL,
  `nome_atendente` VARCHAR(150) NULL,
  `atualizado_em` DATETIME NULL,
  `resolvido_em`  DATETIME NULL,
  `ultimo_email_em` DATETIME NULL,
  `historico_log` TEXT NULL,
  PRIMARY KEY (`id`),
...
-- ------------------------------------------------------------
-- 15. lista_espera_laboratorio
-- Fonte: app/Models/ListaEspera.php, app/Config/lista_espera_schema.php
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- 16. php_sessions + auth_remember_tokens (sessão persistente)
-- Fonte: app/Config/session_bootstrap.php
-- ------------------------------------------------------------
CREATE TABLE IF NOT EXISTS `php_sessions` (
  `id` VARCHAR(128) NOT NULL PRIMARY KEY,
  `data` TEXT NOT NULL,
  `last_access` DATETIME NOT NULL,
  KEY `idx_php_sessions_last` (`last_access`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
